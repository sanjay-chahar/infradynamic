...
if not os.path.exists(path):
    os.mkdir(path)
    print("templates path '{}' created ".format(path))
else:
    print("templates path {} already exists".format(path))
my_file = 'my_first_template'
new_templates_path = os.path.join(path, my_file)
print("new template file name is {}" .format (new_templates_path))
...
print("Number of tab in sheet are {} and list mentioned below \n{}.".format(num_tab_sheet, list_tab_sheet.sheet_names))
...
data = {"first_var" : df.iloc[1, 2], "second_var" : df.iloc[2, 2], "third_var" : df.iloc[3, 2], "fourth_var" : df.iloc[4, 2], "fourth_var" : df.iloc[4, 2], "fifth_var" : df.iloc[4, 2], "sixth_var" : df.iloc[4, 2], "seventh_var" : df.iloc[4, 2], "eighth_var" : df.iloc[4, 2], "nineth_var" : df.iloc[4, 2]}
...
in_interface_config = interface_config
